...
def _eval_rl_episode(model, targets_path, cloud_json_path,
    ...
    # eval_vec = DummyVecEnv([lambda: build_env(seed=EVAL_SEED, event_rate=2.0, use_safety=True)])
    # eval_vec = VecNormalize.load(os.path.join(MODELS, f"vecnormalize_s{seed}.pkl"), eval_vec)
    # eval_vec.training = False        # freeze running stats
    # eval_vec.norm_reward = False     # report RAW reward
    ...
    while not done:
        if hasattr(env, "action_masks"):
            ...
            while hasattr(curr, "env"):
                curr = curr.env
                if hasattr(curr, "action_masks"):
                    mask = curr.action_masks()
                    break
                if hasattr(curr, "get_action_mask"):
                    mask = curr.get_action_mask()
                    break
    ...
    # Unwrap environment to reach the satellite object safely
    eval_unwrapped = env
    while hasattr(eval_unwrapped, "env"):
        eval_unwrapped = eval_unwrapped.env
    sat = eval_unwrapped.unwrapped.satellites[0]
    m   = sat.get_metrics()
...
def evaluate_all_scenarios(
    ...
    if model_path and os.path.exists(model_path):
        try:
            ...
            custom_objects = {"features_extractor_class": SchedulerAttentionExtractor}
            model = MaskablePPO.load(model_path, custom_objects=custom_objects)
            if verbose:
                print(f"  [RL] Loaded MaskablePPO model from {model_path}")
        except Exception as exc:
            try:
                from stable_baselines3 import PPO
                model = PPO.load(model_path)
                if verbose:
                    print(f"  [RL] Loaded standard PPO model from {model_path}")
            except Exception as exc2:
                print(f"  [WARN] Could not load RL model: {exc} (fallback: {exc2})")
...
def main() -> None:
    ...
    ap.add_argument("--model", default=os.path.join(_ROOT, "models/ppo_full_system_s42.zip"),
                help="Path to trained PPO model (.zip). Skipped if not found.")
    ap.add_argument("--targets", default=os.path.join(_ROOT, "config/targets/global_45_targets.json"))
    ap.add_argument("--cloud", default=os.path.join(_ROOT, "config/cloud_reality/global_45_clouds.json"))

...
def collect_background_states(
    ...
    env  = make_env(cfg, targets_path, cloud_json_path,
                    event_rate=event_rate, seed=seed, with_action_mask=True)
...
def rollout_with_logging(
    ...
    try:
        ...
        n_st = len(sat.scenario.targets)
    except Exception:
        pass
        
    dec_log   = DecisionLogger(n_static=n_st, n_dyn=3)
    done      = False
    ...
    while not done:
        ...
        try:
            ...
            with torch.no_grad():
                ...
                if mask is not None and hasattr(model.policy, "get_distribution"):
                    mask_tensor = torch.as_tensor(mask).reshape(1, -1)
                    try:
                        dist = model.policy.get_distribution(t, mask_tensor)
                    except Exception:
...
def main():
    ...
    ap.add_argument("--targets",    default=os.path.join(_ROOT,
                    "config/targets/global_45_targets.json"))
    ap.add_argument("--cloud",      default=os.path.join(_ROOT,
                    "config/cloud_reality/global_45_clouds.json"))
    ...
    try:
        ...
        model = MaskablePPO.load(args.model, custom_objects=custom_objects)
        print(f"  Loaded MaskablePPO model from {args.model}")
    except Exception as exc:
        try:
            from stable_baselines3 import PPO
            model = PPO.load(args.model)
            print(f"  Loaded standard PPO model from {args.model}")
        except Exception as exc2:
            print(f"  [ERROR] Could not load model: {exc} (fallback: {exc2})")
            sys.exit(1)

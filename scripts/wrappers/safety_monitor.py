...
SLEW_PEAK_W       = 100.0
BATTERY_WH        = 300.0
SAFETY_SOC_MARGIN = 0.10    # 5 % extra margin above absolute minimum

# Default drift action index fallback (dynamically updated in check/step)
DRIFT_ACTION      = 48
...
class SafetyMonitor:
    ...
    def check(self,
        ...
        self._n_checked += 1
        n_static = 45
        if satellite is not None and hasattr(satellite, "scenario") and satellite.scenario is not None:
            n_static = len(satellite.scenario.targets)
        self.drift_action = n_static + 3  # N_STATIC + N_DYN_SLOTS
...
class SafetyWrapper(gym.Wrapper):
    ...
    def step(self, action: int):
        ...
        drift_act = self.action_space.n - 1 if hasattr(self, "action_space") else DRIFT_ACTION
        final_action = action if safe else drift_act

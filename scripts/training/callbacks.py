    ...
    def _on_step(self) -> bool:
        ...
        for info in self.locals.get("infos", []):
            ...
            if ep_info is not None:
                ...
                self._ema_avg = 0.90 * self._ema_avg + 0.03 * _raw_avg
        ...
        for info in self.locals.get("infos", []):
            ...
            if self.verbose >= 1 and self._episode % max(1, self.log_every) == 0:
                ...
                try:
                    ...
                    try:
                        e = _unwrap_env(self.training_env)
                        sat = e.satellites[0] if e is not None and hasattr(e, "satellites") else None
                        n_static = len(sat.scenario.targets) if sat is not None else 45
                        drift_act = n_static + 3
                    except Exception:
                        n_static = 45
                        drift_act = 48
                    _st   = sum(v for k, v in _ac.items() if k < n_static) / _n
                    _dy   = sum(v for k, v in _ac.items() if n_static <= k < drift_act) / _n
                    _dr   = _ac.get(drift_act, 0) / _n
...
class VerboseStepLogger(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        try:
            ...
            n_static = len(sat.scenario.targets) if sat else 45
            drift_act = n_static + 3  # drift is always n_static + n_dyn_slots
            ...
            elif n_static <= act < drift_act:
                # ── DYN slot ───────────────────────────────────────────────
                slot_idx = act - n_static
                label, detail = self._dyn_label(sat, slot_idx, rew, now, info)
                print(hdr)
                print(f"  ACT={act:2d} DYN-slot{slot_idx} {detail}  τ={tau_s:.0f}s  "
                      f"r={rew:+.4f} [base={base_r:+.4f} shp={shaping:+.4f}]  {label}")

            elif act == drift_act and self.show_drift:
                batt = "?"
                try:
                    ...
                    pass
                print(hdr)
                print(f"  ACT={act:2d} DRIFT  τ={tau_s:.0f}s  batt={batt}  r={rew:+.4f}  💤")
    ...
    def _get_sat(self):
        try:
            e = _unwrap_env(self.training_env)
            if e is not None and hasattr(e, "satellites"):
    ...
    def _dyn_label(self, sat, slot_idx: int, rew: float, now: float, info: dict):
        ...
        try:
            ...
            imaged_ok = (info is not None and info.get("dyn_event_imaged") is not None)
            if imaged_ok:
                label = "✅ IMAGED"
            elif cf_truth >= CLOUD_THRESH:
                label = f"❌ CLOUD (truth={cf_truth:.2f})"

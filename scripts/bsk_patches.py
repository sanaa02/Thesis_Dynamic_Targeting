...
def _patch_dyn_event_locking() -> bool:
    ...
    try:
        import sys
        if "env_alsat_dynamic" in sys.modules:
            mod = sys.modules["env_alsat_dynamic"]
            if hasattr(mod, "DynamicImageTargetAction"):
                DynamicImageTargetAction = mod.DynamicImageTargetAction
            else:
                return False   # Skip for now, let it be patched at the bottom of env_alsat_dynamic.py
        else:
        ...
        if not hasattr(ImagingSatellite.task_target_for_imaging, "__wrapped__"):
            _orig_task = ImagingSatellite.task_target_for_imaging
            @functools.wraps(_orig_task)
            def _patched_task(self, target, max_duration=None):
                if getattr(self, "_last_tasked_target", None) is target:
                    self.logger.info(f"{target} already tasked, skipping FSW reset")
                    return
                self._last_tasked_target = target
                self._dyn_tasked = True
                return _orig_task(self, target, max_duration)
            ImagingSatellite.task_target_for_imaging = _patched_task
            logger.info("[bsk_patches] Patched ImagingSatellite.task_target_for_imaging to track tasking")

        # Extract the underlying __wrapped__ method if already patched to avoid recursive nesting
        if hasattr(DynamicImageTargetAction.set_action, "__wrapped__"):
            _orig = DynamicImageTargetAction.set_action.__wrapped__
        else:
        ...
        def _locked(self, action: int, prev_action_key=None) -> None:
            sat      = self.satellite
            action_changed = getattr(sat, "_last_action", None) != action
            sat._last_action = action
            ...
            if action < n_static:
                ...
                if monitor is not None:
                    try:
                        _tgt = sat.scenario.targets[action]
                        now  = float(sat.simulator.sim_time)
                        _chk = monitor.check(sat, action, _tgt, now)
                        safe = _chk[0] if isinstance(_chk, tuple) else bool(_chk)
                        if not safe:
                            logger.info(f"[Patch4] Safety veto: static target {_tgt.name} (action={action}) is unsafe -> redirecting to DRIFT")
                            self.set_action(n_static + _NDYN, prev_action_key)
                            return
                    except Exception as _e_safe:
                        logger.warning(f"Error checking safety for static target in patch: {_e_safe}")
                if action_changed:
                    sat._locked_dyn_slot  = -1
                    sat._locked_dyn_event = None
                    sat._dyn_img_fired    = False
                    sat._min_dyn_slew     = float('inf')
                    sat._dyn_tasked       = False
                    sat._last_tasked_target = None
                return _orig(self, action, prev_action_key)
            if action >= n_static + _NDYN:
                if action_changed:
                    sat._locked_dyn_slot  = -1
                    sat._locked_dyn_event = None
                    sat._dyn_img_fired    = False
            ...
            if locked_slot == slot and locked_event is not None:
                try:
                    ...
                    if monitor:
                        now  = float(sat.simulator.sim_time)
                        _chk = monitor.check(sat, action, locked_event, now)
                        safe = _chk[0] if isinstance(_chk, tuple) else bool(_chk)
                        if not safe:
                            logger.info(f"[Patch4] Safety veto: dynamic event {locked_event.name} (action={action}) is unsafe -> redirecting to DRIFT")
                            self.set_action(n_static + _NDYN, prev_action_key)
                            return
                    try:
                        import numpy as np
                        ...
                        r_tgt = np.asarray(locked_event.r_LP_P, dtype=float).ravel()
                        los   = r_tgt - r_sat
                    ...
                    if off_nadir <= math.radians(45.0):
                        ...
                        try:
                            ...
                            _fake  = {"object": locked_event,
                                      ...
                                      "type": "target", "r_LP_P": getattr(locked_event, 'r_LP_P', None)}
                            if not hasattr(sat, "opportunities"):
                                    sat.opportunities = []
                            sat.opportunities = [o for o in sat.opportunities if o.get("object") is not locked_event]
                            import bisect
                            bisect.insort(
                                sat.opportunities,
                                _fake,
                        ...
                        try:
                            if not getattr(sat, "_dyn_tasked", False):
...
def _patch_force_dyn_imaging() -> bool:
    ...
    try:
        ...
        for name in dir(_acc):
            ...
            if hasattr(cls.was_image_taken_since_last_check, "__wrapped__"):
        ...
        for _mname, _mod in list(_sys.modules.items()):
            ...
            for _cname in dir(_mod):
                try:
                    ...
                    if hasattr(_cls.was_image_taken_since_last_check, "__wrapped__"):
        ...
        try:
            ...
            if (id(AlsatSatellite) not in _patched_classes
                ...
                if hasattr(AlsatSatellite.was_image_taken_since_last_check, "__wrapped__"):

...
class EventGenerator:
    ...
    def __init__(self,
        ...
        self._scripted: list = [] # real events released by appearance_time

    # ── Public API ────────────────────────────────────────────────────────────

    def step(self, sim_time: float, dt: float) -> List[DynamicEvent]:
        new_events: List[DynamicEvent] = []
        # Poisson (synthetic) arrivals
        remaining_dt = dt
        ...
        # Scripted (real) arrivals due by now
        if self._scripted:
            due = [e for e in self._scripted if e.appearance_time <= sim_time]
            for e in due:
                new_events.append(e)
                self._scripted.remove(e)
        return new_events
    ...
    def add_scripted(self, events: list) -> None:
        """Queue real events; released by step() when sim_time >= appearance_time."""
        self._scripted.extend(events)
        self._scripted.sort(key=lambda e: e.appearance_time)
...
class EventManager:
    ...
    def get_slots(self, satellite, sim_time: float) -> list:
        ...
        active = [
            ...
            if not e.imaged
            and getattr(e, "appearance_time", 0.0) <= sim_time
            and e.expiration_time > sim_time
        ]
        if not active:
...
def _slew_angle_safe(satellite, target) -> float:
    ...
    try:
        ...
        r_sat = np.asarray(satellite.dynamics.r_BN_P, dtype=float).ravel()

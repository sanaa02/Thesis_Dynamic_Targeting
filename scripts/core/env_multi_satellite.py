...
class ClaimRegistry:
    ...
    def __init__(self):
        ...
        self._claimed_by:    dict = {}      # event_id → sat_id
    ...
    def try_claim_event(
        ...
        """
        Returns True if claim was successful (event not already taken by another sat).
        Same satellite may re-claim its own event (idempotent).
        """
        if event_id in self._claimed_events:
            existing_sat = self._claimed_by.get(event_id)
            return existing_sat == sat_id   # same sat can re-claim
        self._claimed_events.add(event_id)
        self._claimed_by[event_id] = sat_id
        return True
    ...
    def step_wait(self) -> VecEnvStepReturn:
        ...
        for i, (env, action) in enumerate(zip(self.envs, actions)):
            ...
            if 0 <= action < N_STATIC:
                ...
                if not self._claim_reg.try_claim_event(f"sat_{sat_idx}", event_id):
                    rew = 0.0
                    info["double_imaging_penalty"] = True

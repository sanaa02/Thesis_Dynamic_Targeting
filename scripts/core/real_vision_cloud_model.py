...
Architecture
------------
  truth   <- MODIS JSON (global_45_clouds.json)   [ground truth for reward]
...
----------------
    ...
    model = RealVisionCloudModel(
        cloud_json_path = "config/cloud_reality/global_45_clouds.json",
...
class RealVisionCloudModel:
    ...
    ----------
    cloud_json_path : str
        Path to global_45_clouds.json (provides ground truth).
    patches_dir : str
    ...
    def __init__(
        ...
        self._target_names  = target_names or {}  # dict: {target_id: "name"}
        self._episode_date  = None  # set each episode via set_episode_date()
    ...
    def forecast(
        self, target_id: int, sim_time_s: float) : #-> Tuple[float, float]
        """
        ...
        # Try to load the specific dated patch for this target
        patch = None
        target_name = self._target_names.get(int(target_id))
        if target_name and self._episode_date and self._model is not None:
            from scripts.data_fetchers.fetch_modis_patches import load_patch
            patch = load_patch(target_name, self._episode_date,
                            patch_dir=self._patches_dir)
            # load_patch returns synthetic fallback if file missing - always valid

        if patch is None:
            # fallback to random pool patch
            patch = self._provider.get_patch() if self._model is not None else None

        if patch is not None and self._model is not None:
            try:
                import torch
                # patch is (H, W) from load_patch - needs (3, H, W)
                if patch.ndim == 2:
                    patch = np.stack([patch, patch, patch], axis=0)
                t = torch.from_numpy(patch.astype(np.float32)).unsqueeze(0).to(self._device)
                with torch.no_grad():
    ...
    def set_episode_date(self, date_str: str) -> None:
        """Called on each env.reset() to align patches with episode date."""

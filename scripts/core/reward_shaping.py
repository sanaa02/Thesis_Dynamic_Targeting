...
N_STATIC = 45  # FIX: env loads global_45_targets.json (was 20)
N_DYN    = 3
DRIFT    = 23
...
class DynamicRewardShaper(gym.Wrapper):
    ...
    def __init__(
        ...
        explore_bonus_init: float = 0.01,
        explore_decay:      float = 0.98,   # Decays explore bonus over ~150-200 dynamic eps
        explore_min:        float = 0.0,    # Floor decays to 0.0 so failed actions become net negative
        attempt_bonus:      float = 0.0,      # FIX-RS-4: new
    ):
        super().__init__(env)
        ...
        try:
            ...
            self._n_static = len(sat.scenario.targets)
        except Exception:
            pass
        try:
            from dynamic_event import N_DYN_SLOTS
            self._n_dyn = int(N_DYN_SLOTS)
        except Exception:
            pass
    ...
    def reset(self, **kw):
        ...
        # Only decay explore bonus if dynamic events are active (Stage 2+)
        # This preserves the full explore_bonus bootstrap capacity until dynamic targeting begins.
        has_dyn = False
        try:
            curr = self.env
            while curr is not None:
                if hasattr(curr, "_gen") and curr._gen is not None:
                    if getattr(curr._gen, "rate_hz", 0.0) > 0.0:
                        has_dyn = True
                    break
        ...
        # FIX-4: reset per-episode static imaged set for diversity tracking
        self._ep_imaged_static: set = set()
        return result
    ...
    def step(self, action: int):
        ...
        # FIX-4 (replaces PATCH 7): per-target uniqueness bonus
        # Original PATCH 7 gave flat +0.5 to ALL static imaging when ep_count<200,
        # which gave no diversity incentive — agent maximised by re-imaging the
        # single most-accessible target (Constantine) over and over.
        # New logic:
        #   • Base shaping bonus: +0.25 for any successful static image (ep<200)
        #   • Diversity bonus:    +0.30 EXTRA for the FIRST image of each unique
        #     target per episode (always active — does not decay)
        # Together these encourage breadth while still bootstrapping Stage 0.
        _N_STATIC = getattr(self, "_n_static", 45)  # FIX: 20 -> 45
        if action < _N_STATIC and float(reward) > 0.05:
            # Base shaping — decays after ep 200
            if getattr(self, "_ep_count", 0) < 200:
                reward = float(reward) + 0.25

            # Diversity bonus — first imaging of this target this episode
            _ep_imaged = getattr(self, "_ep_imaged_static", None)
            if _ep_imaged is None:
                self._ep_imaged_static = set()
                _ep_imaged = self._ep_imaged_static
            if action not in _ep_imaged:
                _ep_imaged.add(action)
                reward = float(reward) + 0.30  # diversity bonus
    ...
    def _bonus(self, action: int, base_reward: float, info: dict) -> float:
        n_static = getattr(self, "_n_static", 45)
        n_dyn = getattr(self, "_n_dyn", 3)
    ...
    def _detect_dyn_success(self, info: dict, base_reward: float) -> bool:
        """
        FIX-RS-2: detect successful DYN imaging (clean, cloud-free only).

        Strategy A: info key "dynamic_imaging_occurred" (set by env_alsat_dynamic.py only on clean image)
        Strategy B: detect increment in n_dyn_imaged_clean counter
        Strategy C: base_reward > threshold (heuristic fallback)
        """
        success = False
        # Strategy A (preferred — set in env_alsat_dynamic.py when cloud-free)
        if info.get("dynamic_imaging_occurred", False):
            success = True

        # Strategy B: detect increment in n_dyn_imaged_clean counter
        # We read from episode_metrics where n_dyn_imaged_clean is stored.
        em = info.get("episode_metrics", {})
        cur_clean = int(em.get("n_dyn_imaged_clean", self._prev_dyn_imaged))
        if cur_clean > self._prev_dyn_imaged:
            success = True
        self._prev_dyn_imaged = cur_clean

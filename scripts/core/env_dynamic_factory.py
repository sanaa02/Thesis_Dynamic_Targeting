...
def _make_cloud_model(
    ...
    if cfg == Config.DYN_REAL_VISION:
        try:
            ...
            from env_alsat_debug import load_targets_config

            # Build target_id -> name mapping
            targets_cfg = load_targets_config(
                os.path.join(os.path.dirname(cloud_json_path),
                             "..", "targets", "global_45_targets.json"))
            target_names = {i: cfg_t.get("name", f"target_{i}")
                            for i, cfg_t in enumerate(targets_cfg)}
...
def make_env(
    ...
    print(f"[DEBUG] make_env: event_rate={event_rate}")

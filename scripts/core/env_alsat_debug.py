...
BATTERY_SOC0   = 1.0       # 100% initial SOC
...
# Eclipse-aware power model.
# Average net power over SSO orbit at 686 km (sunlit fraction = 65.3%).
# Basilisk's solar panel module requires a connected sun-direction message
# that is not configured in this setup, so it outputs 0 W. The correct
# orbit-averaged net power is modelled via basePowerDraw instead:
#
#   P_solar_avg = 1.5 m² × 0.28 × 1367 W/m² × 0.65 × 0.653 sunlit = 245 W
#   P_net_avg   = 245 - 20 = +225 W  →  battery stays near 100 %, correct for SSO
#
# Realistic eclipse-cycle variation (~96–100 %) requires a working
# Basilisk EclipseModel + SimpleSolarPanel connection (future work).
...
OE_ALSAT.Omega = math.radians(240) 
OE_ALSAT.omega = math.radians(90.0)
OE_ALSAT.f     = math.radians(270)
...
class TorqueLimitedDynamics(ImagingDynModel):
    # Power parameters — used by _create_power_system below
    panelArea: float          = 1.5        # m²
    panelEfficiency: float    = 0.28       # BOL GaAs
    batteryCapacity: float    = 300.0 * 3600.0      # 300 Wh in Joules
    batteryInitialCharge: float = 255.0 * 3600.0    # 85% of 300 Wh
    housekeepingPower: float  = 50.0       # W, always-on load
    imagingPower: float       = 300.0      # W, camera load during imaging
    ...
    def _create_power_system(self) -> None:
        """
        Wire up the real Basilisk power system:
          SimpleSolarPanel → subscribes to spacecraft state + sun direction
          EclipseModel     → provides sunlight fraction to solar panel
          SimpleBattery    → accumulates power from panel, discharges through sinks
          housekeepingSink → always-on 50 W load
          imagingSink      → 300 W camera load, toggled by set_imaging_power()
        """
        try:
            from Basilisk.simulation import (
                simpleSolarPanel, simpleBattery, simplePowerSink
            )
            ...
            nHat_B = [0, 0, -1]   # panel faces nadir (away from body +Z)
            panel.setPanelParameters(nHat_B, self.panelArea, self.panelEfficiency)
            panel.stateInMsg.subscribeTo(self.scObject.scStateOutMsg)

            # Sun direction from SPICE (gravFactory sets this up)
            spice_obj = getattr(getattr(world, 'gravFactory', None), 'spiceObject', None)
            if spice_obj is not None and hasattr(spice_obj, 'planetStateOutMsgs'):
                panel.sunInMsg.subscribeTo(spice_obj.planetStateOutMsgs[0])
            else:
                # Fallback: use a constant sun message pointing along +X inertial
                from Basilisk.architecture import messaging
                sunMsg = messaging.SpicePlanetStateMsgPayload()
                sunMsg.PositionVector = [1.5e11, 0.0, 0.0]  # 1 AU along X
                sunMsg.VelocityVector = [0.0, 0.0, 0.0]
                self._sunMsgBuf = messaging.SpicePlanetStateMsg().write(sunMsg)
                panel.sunInMsg.subscribeTo(self._sunMsgBuf)

            # Eclipse model feeds the panel
            eclipse_obj = getattr(world, 'eclipseObject', None)
            if eclipse_obj is not None and hasattr(eclipse_obj, 'eclipseOutMsg'):
                panel.sunEclipseInMsg.subscribeTo(eclipse_obj.eclipseOutMsg)

            self.simulator.AddModelToTask(self.task_name, panel, ModelPriority=1000)
            self.solarPanel = panel

            # ── Battery ───────────────────────────────────────────────────
            battery = simpleBattery.SimpleBattery()
            battery.ModelTag           = "battery"
            battery.storageCapacity    = self.batteryCapacity
            battery.storedCharge_Init  = self.batteryInitialCharge
            battery.addPowerNodeToModel(panel.nodePowerOutMsg)
            self.simulator.AddModelToTask(self.task_name, battery, ModelPriority=1001)
            self.powerMonitor = battery          # expose for bsk_rl

            # ── Housekeeping sink (always on) ─────────────────────────────
            hk_sink = simplePowerSink.SimplePowerSink()
            hk_sink.ModelTag     = "housekeepingSink"
            hk_sink.nodePowerOut = -self.housekeepingPower   # negative = draw
            hk_sink.powerStatus  = 1
            battery.addPowerNodeToModel(hk_sink.nodePowerOutMsg)
            self.simulator.AddModelToTask(self.task_name, hk_sink, ModelPriority=1002)

            # ── Imaging sink (toggle on/off) ──────────────────────────────
            img_sink = simplePowerSink.SimplePowerSink()
            img_sink.ModelTag     = "imagingSink"
            img_sink.nodePowerOut = -self.imagingPower   # negative = draw
            img_sink.powerStatus  = 0                    # off by default
            battery.addPowerNodeToModel(img_sink.nodePowerOutMsg)
            self.simulator.AddModelToTask(self.task_name, img_sink, ModelPriority=1003)
            self.imagingSink = img_sink

            print("[POWER] Real Basilisk power system connected: "
                  f"panel={self.panelArea}m²  battery={self.batteryCapacity/3600:.0f}Wh  "
                  f"HK={self.housekeepingPower}W  cam={self.imagingPower}W")

        except Exception as e:
            print(f"[POWER] WARNING: power system wiring failed ({e}) — "
                  "battery will stay at initial SOC")

    def set_imaging_power(self, is_imaging: bool) -> None:
        """Enable/disable the camera power sink."""
        if hasattr(self, 'imagingSink'):
            self.imagingSink.powerStatus = 1 if is_imaging else 0
...
class AlsatSatellite(ImagingSatellite):
    sat_args_default = dict(
        ...
        panelArea              = 1.5,
        panelEfficiency        = 0.28,
        basePowerDraw          = 0.0,   # +226 W net (solar-HK) NET_BASE_POWER_W
    ...
    def reset_post_sim_init(self):
        ...
        # ── Add EclipseWorldModel ──────────────────────────────────────────
        try:
            from bsk_rl.sim.world import EclipseWorldModel
            world = self.simulator.world
            if not hasattr(world, 'eclipseObject'):
                eclipse = EclipseWorldModel()
                world.add_world_object(eclipse)
                eclipse.addSpacecraftToModel(self.dynamics.scObject.scStateOutMsg)
                print("[ECLIPSE] EclipseWorldModel added.")
        except Exception as e:
            print(f"[ECLIPSE] Could not add eclipse: {e}")
        ...
        # print(f"[DEBUG] Power dynamics available: {hasattr(self.dynamics, 'power')}")
        if hasattr(self.dynamics, 'power'):
            print(f"[DEBUG] Panel area: {self.dynamics.power.panelArea}")
            print(f"[DEBUG] Panel efficiency: {self.dynamics.power.panelEfficiency}")
...
def make_env(
    ...
    satellite = AlsatSatellite(
        ...
        initial_generation_duration= SIM_DURATION_S,     #gen_duration + 7200
    )
...
if __name__ == "__main__":
    ...
    TARGETS    = os.path.join(ROOT, "config/targets/targets/global_45_targets.json")
    ...
    for step in range(5):
        ...
        if term or trunc:
            print(f"Episode ended at t={sat.simulator.sim_time/3600:.1f}h, term={term}, trunc={trunc}")
            print(f"  upcoming_opportunities: {len(sat.upcoming_opportunities)}")
            print(f"  generation_duration: {sat.generation_duration/3600:.1f}h")
            print(f"  time_limit: {env.unwrapped.time_limit/3600:.1f}h")
        total_r += r

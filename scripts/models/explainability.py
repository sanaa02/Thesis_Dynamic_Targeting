...
class TimelineRenderer:
    ...
    def render(self, records: List[DecisionRecord],
        ...
        # Dynamically determine n_static
        n_st = n_static
        if n_st is None:
            try:
                for r in records:
                    if r.policy_probs is not None:
                        n_st = len(r.policy_probs) - (n_dyn + 1)
                        break
            except Exception:
                pass
        if n_st is None:
            # Fallback based on maximum action observed
            max_act = max((r.action for r in records), default=0)
            if max_act >= 23:
                n_st = 45
            else:
                n_st = 20

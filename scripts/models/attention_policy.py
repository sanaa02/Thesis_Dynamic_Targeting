...
_N_TF      = 6    # props per static slot (priority,cloud,std,opp_open,slew,target_id)
_N_DS      = 3    # N_DYN_SLOTS
_N_DF      = 4    # features per dynamic slot
_N_SOJOURN = 1
# indices in obs vector
_IDX_STATE_END   = _N_STATE                          # 13
_IDX_TARGET_END  = _IDX_STATE_END + _N_TS * _N_TF   # 49  (13 + 6×6)
...
def make_attention_ppo(vec_env,
                       ...
                       n_steps:      int   = 2048,
                       batch_size:   int   = 64,
                       n_epochs:     int   = 5,       # was 10 — reduced slightly
                       gamma:        float = 0.99,
                       ent_coef:     float = 0.05,    # ← KEY: was 0.01 — prevents collapse
                       seed:         int   = 42,
                       device:       str   = "cpu"):
    """
    Build MaskablePPO with the SchedulerAttentionExtractor features extractor.
 
    Changes from original:
      net_arch  = [128]   — one hidden layer between 256-dim features and logits.
                            Without this the actor/critic head is a single linear
                            projection, insufficient for a 24-action space.
                            (Herrmann & Schaub 2023 use similar depth.)
      ent_coef  = 0.05   — start higher to prevent the entropy collapse observed
                            in training logs (entropy → 0 within 30K steps).
                            EntropyAnnealingCallback decays this to 0.005.
      clip_range = 0.15  — tighter than SB3 default to stabilise learning when
                            SMDP rewards are large and variable.
      n_epochs   = 5     — slightly fewer epochs per rollout to avoid overfitting
                            the small SMDP rollout buffer.
    """
    ...
    policy_kwargs = dict(
        ...
        net_arch = [128],   # ← CHANGED: was [] (no hidden layer)
    )
    ...
    model = PPO(
        ...
        vf_coef       = 0.5,
        clip_range    = 0.15,               # ← NEW: was SB3 default 0.2
        max_grad_norm = 0.5,
        verbose       = 0,
        seed          = seed,
        device        = device,
